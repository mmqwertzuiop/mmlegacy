    ...
    <div>
      ...
      <div style={{ marginBottom: '32px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '48px', color: 'var(--ghost)', letterSpacing: '0.04em' }}>VÝKUP KARIET</h1>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ padding: '60px', textAlign: 'center', border: '1px solid var(--surface-2)', background: 'var(--surface)' }}>
          <p style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '36px', color: 'var(--dim)', letterSpacing: '0.04em', marginBottom: '8px' }}>ŽIADNE ŽIADOSTI</p>
          <p style={{ fontFamily: 'Space Mono, monospace', fontSize: '10px', color: 'var(--dim)' }}>Zatiaľ nebola odoslaná žiadna výkup ponuka.</p>
        </div>
      ) : (
        <div style={{ border: '1px solid var(--surface-2)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '150px 1fr 200px 60px 110px 110px 100px', background: 'var(--surface-2)', padding: '10px 16px', gap: '8px' }}>
          ...
          {filtered.map((s, i) => (
            <div key={s.id} onClick={() => openDetail(s)}
              style={{
                display: 'grid', gridTemplateColumns: '150px 1fr 200px 60px 110px 110px 100px',
                padding: '14px 16px', gap: '8px',
                borderBottom: i < filtered.length - 1 ? '1px solid var(--surface-2)' : 'none',
                background: 'transparent',
                cursor: 'none', alignItems: 'center', transition: 'background 0.15s',
              }}
                ...
                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                  {(s.buyback_items || []).slice(0, 3).map((item, j) => (
                    <span key={j} style={{ fontFamily: 'Space Mono, monospace', fontSize: '9px', color: 'var(--dim)', background: 'var(--surface-2)', padding: '2px 6px' }}>
                      {item.card_name.split(' ').slice(0, 3).join(' ')}{item.is_graded && item.psa_grade ? ` PSA${item.psa_grade}` : ''}{item.quantity > 1 ? ` ×${item.quantity}` : ''}
                    </span>
                  ))}
              ...
              <span style={{ fontFamily: 'Space Mono, monospace', fontSize: '9px', letterSpacing: '0.08em', color: STATUS_LABELS[s.status]?.color || 'var(--dim)', padding: '3px 8px', border: `1px solid ${STATUS_LABELS[s.status]?.color || 'var(--dim)'}` }}>
      ...
      {selected && (
        <div
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 100, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 24px', overflowY: 'auto' }}
          onClick={e => e.target === e.currentTarget && setSelected(null)}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--surface-2)', width: '100%', maxWidth: '720px', padding: '40px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', marginBottom: '28px' }}>
              {(selected.buyback_items || []).map((item) => (
                <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 130px', gap: '8px', padding: '12px 16px', background: 'var(--void)', border: '1px solid var(--surface-2)', alignItems: 'center' }}>
                  <div>
                    ...
                    <p style={{ fontFamily: 'Space Mono, monospace', fontSize: '9px', color: 'var(--dim)' }}>
                      {item.card_set.split('(')[0].trim()} · {item.card_condition.split(' ')[0]} · {item.card_language}
                      {item.quantity > 1 && <span style={{ color: 'var(--orange)' }}> · {item.quantity}ks</span>}
